...
def zip_files(output_dir, file_names, zip_name, package_dir=None):
    package_dir = package_dir or output_dir
    os.makedirs(package_dir, exist_ok=True)
    zip_path = os.path.join(package_dir, zip_name)
    with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
        for file_name in file_names:
            zipf.write(os.path.join(output_dir, file_name), arcname=os.path.basename(file_name))
    return zip_path
...
def write_reference_assets_to_csv(output_dir, filepath, rows, header=['core.externalId', 'core.Reference', 'core.assignable', 'core.name']):
    ...
    existing_rows = set()
    if file_exists:
        # Check if header exists and read existing rows
    ...
    # Filter both against existing rows and against duplicates in the new rows
    unique_new_rows = []
    seen_new_rows = set()
    for row in rows:
        row_tuple = tuple(row)
        if row_tuple not in existing_rows and row_tuple not in seen_new_rows:
            unique_new_rows.append(row)
            seen_new_rows.add(row_tuple)

    if not unique_new_rows and not write_header:
        # Nothing to write
        return
    ...
    with open(full_path, 'a', newline='', encoding='utf-8') as f:
        ...
        writer.writerows(unique_new_rows)
